...
let dsgen;
beforeEach(() => {
    const fuelSource = new Tank_1.default('dummy fuel tank', 100, startFuelAmount);
    const dummyFuelOutletValve = new Valve_1.default('test fuel source outlet valve', fuelSource);
    dummyFuelOutletValve.Open();
    const lubSource = new Tank_1.default('dummy lub  tank', 100, startLubAmount);
...
describe('Slump', () => {
    ...
    test('Lub source is empty, stop adding slump', () => {
        dsgen.LubProvider = new Tank_1.default('dummy', 100, Cst_1.CstPowerSys.DsGen1.Slump.TankAddStep);
        dsgen.LubIntakeValve.Open();
        dsgen.Thick();
        expect(dsgen.LubSlump.AddEachStep).toBe(Cst_1.CstPowerSys.DsGen1.Slump.TankAddStep);
        expect(dsgen.LubSlump.Content).toBe(Cst_1.CstPowerSys.DsGen1.Slump.TankAddStep);
