...
let fuelSource;
let lubSource;
const airSource = new Tank_1.default('dummy air receiver', 100, startAirAmount);
beforeEach(() => {
    fuelSource = new Tank_1.default('dummy fuel tank', 100, startFuelAmount);
    const dummyFuelOutletValve = new Valve_1.default('test fuel source outlet valve', fuelSource);
    dummyFuelOutletValve.isOpen = true;
    lubSource = new Tank_1.default('dummy lub  tank', 100, startLubAmount);
    const dummyLubOutletValve = new Valve_1.default('test lub source outlet valve', lubSource);
    const dummyAirOutletValve = new Valve_1.default('test air source valve', airSource);
    const dummyLubCooler = new Cooler_1.default('dummy cooler', 1);
...
describe('Slump', () => {
    ...
    test('Lub source is empty, stop adding slump', () => {
        dsgen.LubIntakeValve.Source = new Tank_1.default('dummy', 100, Cst_1.CstPowerSys.DsGen1.Slump.TankAddStep);
        dsgen.LubIntakeValve.Open();
        dsgen.Thick();
        ...
        dsgen.LubIntakeValve.Source = new Tank_1.default('dummy', 100, 0);
...
describe('Start', () => {
    ...
    test('open fuel valve & has lubrication & min air  = can start = consumes fuel', () => {
        ...
        expect(fuelSource.RemoveEachStep).toBe(Cst_1.CstFuelSys.DieselGenerator.Consumption);
    ...
    test('running and no fuel  = stop', () => {
        ...
        dsgen.Thick();
        expect(dsgen.isRunning).toBeTruthy();
        const emptyFuelSource = new Tank_1.default('empty tank', 100, 0);
        const emptyFuelValve = new Valve_1.default('dummy', emptyFuelSource);
    ...
    test('fuel & lubrication but to less air = cannot start', () => {
        dsgen.FuelIntakeValve.Open();
        dsgen.LubSlump.Inside = Cst_1.CstPowerSys.DsGen1.Slump.MinForLubrication;
        dsgen.AirIntakeValve.Source = new Tank_1.default('dummy', 100, Cst_1.CstAirSys.DieselGenerator.MinPressure - 0.1);
        dsgen.AirIntakeValve.Open();
        dsgen.Start();
        dsgen.Thick();
        expect(dsgen.isRunning).toBeFalsy();
