...
describe('Tank init', () => {
    test('New empty tank', () => {
        ...
        const tank = new Tank_1.default(name, 250);
    ...
    test('New tank with content', () => {
        ...
        const tank = new Tank_1.default(name, 3000, 100);
...
describe('Tank add 1 step', () => {
    test('Add 1 step to empty tank', () => {
        const tank = new Tank_1.default('test tank', 250);
    ...
    test('Add 1 step to not empty tank', () => {
        ...
        const tank = new Tank_1.default('not empty test tank', 1000, start);
    ...
    test('Try overfill tank', () => {
        ...
        const tank = new Tank_1.default('almost full tank', volume, start);
        tank.cbFull = () => {
            cbFullFlag = true;
        };
        tank.AddEachStep = addEachStep;
...
describe('Tank remove 1 step', () => {
    test('Remove from full tank', () => {
        ...
        const tank = new Tank_1.default('full tank', start, start);
    ...
    test('Remove from empty tank, without callback', () => {
        ...
        const tank = new Tank_1.default('empty tank', 200);
        tank.RemoveEachStep = removeEachStep;
        tank.Removing = true;
        tank.Thick();
        expect(tank.Content).toBe(0);
    ...
    test('Remove from more then content from tank = empty (not negative content)', () => {
        const removeEachStep = 80;
        const start = 50;
        const tank = new Tank_1.default('not empty tank', 200, start);
        tank.RemoveEachStep = removeEachStep;
        tank.Removing = true;
        tank.Thick();
        expect(tank.Content).toBe(0);
...
describe('Tank add over time', () => {
    test('Add 4 steps of each 10', () => {
        ...
        const tank = new Tank_1.default('add over time', 100, start);
        tank.AddEachStep = addEachStep;
        tank.Adding = true;
        tank.Thick();
        expect(tank.Content).toBe(start + addEachStep);
    ...
    test('Start filling until full', () => {
        ...
        const maxTank = 100;
        let steps = 0;
        const tank = new Tank_1.default('test tank', maxTank, startContent);
        tank.AddEachStep = addEachStep;
        let cbFullFlag = false;
        const cbFull = () => {
            cbFullFlag = true;
...
describe('Tank remove over time', () => {
    test('Remove in 4 steps of each 10', () => {
        ...
        const tank = new Tank_1.default('remove over time', 100, start);
        tank.RemoveEachStep = removeEachStep;
        tank.Removing = true;
        tank.Thick();
        expect(tank.Content).toBe(start - removeEachStep);
...
describe('Alarms', () => {
    ...
    beforeEach(() => {
        ...
        alarmTank = new Tank_1.default('test alarm tank', 250);
