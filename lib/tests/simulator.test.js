...
describe('Steam', () => {
    test('Ignite boiler', () => {
        ...
        MainBreaker1.Close();
        simulator.Thick();
        expect(MainBus1.Voltage).toBe(Cst_1.CstPowerSys.Voltage);
        // fake full feed water supply (is tested in SteamSys)
        FeedWaterSupply.Tank.Inside = Cst_1.CstSteamSys.FeedWaterSupply.TankVolume;
        // fake full DsService
        DsService.Tank.Inside = Cst_1.CstFuelSys.DsServiceTank.TankVolume;
        DsService.OutletValve.Open();
        ...
        simulator.Thick();
        expect(Boiler.WaterLevel).toBe(Cst_1.CstSteamSys.FeedWaterPump);
        do {
            simulator.Thick();
